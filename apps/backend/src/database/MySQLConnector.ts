...
export class MySQLConnector extends DatabaseConnector {
    ...
    try {
      ...
      if (error.code === 'ECONNREFUSED') {
        throw new ConnectionError(
          `데이터베이스 서버에 연결할 수 없습니다: ${config.host}:${config.port}`,
          config.host,
          config.port
        );
      } else if (error.code === 'ER_ACCESS_DENIED_ERROR') {
        throw new ConnectionError(
          '인증에 실패했습니다. 사용자명과 비밀번호를 확인해주세요.',
          config.host,
          config.port
        );
      } else if (error.code === 'ER_BAD_DB_ERROR') {
        throw new ConnectionError(
          `데이터베이스 '${config.database}'를 찾을 수 없습니다.`,
          config.host,
          config.port
        );
   ...
   * 쿼리 실행
   */
  async executeQuery(query: string, params: any[] = [], options: QueryOptions = {}): Promise<QueryResult> {
    if (!this.connected || !this.pool) {
      throw new ConnectionError('데이터베이스에 연결되지 않았습니다', '', 0);
    }
    ...
    try {
      ...
      const queryResult: QueryResult = {
        ...
        insertId: (result.rows as any)?.insertId
      };
