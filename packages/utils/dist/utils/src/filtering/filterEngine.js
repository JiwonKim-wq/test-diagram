...
export class FilterEngine {
     ...
     */
    applyFilters(data, filters, operator = 'AND') {
     ...
     * 데이터 타입을 검증합니다.
     */
    validateDataType(value, expectedType, fieldName) {
        if (value === null || value === undefined) {
     ...
     * 두 값을 비교합니다.
     */
    compareValues(a, b, caseSensitive = true) {
     ...
     * 문자열 포함 여부를 확인합니다.
     */
    stringContains(str, searchValue, caseSensitive = true) {
     ...
     * 문자열 시작 여부를 확인합니다.
     */
    stringStartsWith(str, searchValue, caseSensitive = true) {
        const strVal = caseSensitive ? String(str) : String(str).toLowerCase();
        const searchVal = caseSensitive ? String(searchValue) : String(searchValue).toLowerCase();
     ...
     * 문자열 끝 여부를 확인합니다.
     */
    stringEndsWith(str, searchValue, caseSensitive = true) {
        const strVal = caseSensitive ? String(str) : String(str).toLowerCase();
        const searchVal = caseSensitive ? String(searchValue) : String(searchValue).toLowerCase();
     ...
     */
    applyFilterGroup(data, filterGroup) {
     ...
     */
    evaluateFilterGroup(item, filterGroup) {
        if (!filterGroup.enabled) {
            return true;
        }
        // 활성화된 규칙들 처리
