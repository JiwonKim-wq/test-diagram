...
export class AggregateEngine {
   ...
   * 데이터를 그룹화합니다.
   */
  private groupData(data: any[], groupBy: string[]): Array<{ key: any; items: any[] }> {
    if (groupBy.length === 0) {
   ...
   * 개별 그룹에 집계 함수를 적용합니다.
   */
  private applyAggregation(items: any[], aggregation: AggregationRule): any {
   ...
   * 표준편차를 계산합니다.
   */
  private calculateStandardDeviation(values: any[]): number | null {
    if (values.length === 0) return null;
   ...
   * 분산을 계산합니다.
   */
  private calculateVariance(values: any[]): number | null {
    if (values.length === 0) return null;
   ...
   * 데이터를 정렬합니다.
   */
  private sortData(data: any[], orderBy: OrderByRule[]): any[] {
