...
// 노드 검증 메인 함수
export function validateNode(node: BaseNode): NodeValidationResult {
  ...
  switch (node.type) {
    case NodeType.DATABASE:
      validateDatabaseNode(node.data as DatabaseNodeData, errors, warnings);
      break;
    case NodeType.FILTER:
      validateFilterNode(node.data as FilterNodeData, errors, warnings);
      break;
    case NodeType.AGGREGATE:
      validateAggregateNode(node.data as AggregateNodeData, errors, warnings);
      break;
    case NodeType.TRANSFORM:
      validateTransformNode(node.data as TransformNodeData, errors, warnings);
      break;
    case NodeType.JOIN:
      validateJoinNode(node.data as JoinNodeData, errors, warnings);
      break;
    case NodeType.OUTPUT:
      validateOutputNode(node.data as OutputNodeData, errors, warnings);
      break;
    case NodeType.PYTHON:
      validatePythonNode(node.data as PythonNodeData, errors, warnings);
      break;
    case NodeType.LOGPRESSO:
      validateLogpressoNode(node.data as LogpressoNodeData, errors, warnings);
      break;
    default:
      errors.push({
        field: 'type',
...
// 기본 노드 검증
function validateBaseNode(node: BaseNode, errors: ValidationError[], warnings: ValidationWarning[]): void {
...
// 데이터베이스 노드 검증
function validateDatabaseNode(data: DatabaseNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
...
// 필터 노드 검증
function validateFilterNode(data: FilterNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
...
// 필터 규칙 검증
function validateFilterRule(rule: FilterRule, fieldPrefix: string, errors: ValidationError[], warnings: ValidationWarning[]): void {
...
// 집계 노드 검증
function validateAggregateNode(data: AggregateNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
...
// 집계 규칙 검증
function validateAggregationRule(rule: AggregationRule, fieldPrefix: string, errors: ValidationError[], warnings: ValidationWarning[]): void {
  if (!rule.field || rule.field.trim() === '') {
    errors.push({
      field: `${fieldPrefix}.field`,
...
// 조인 노드 검증
function validateJoinNode(data: JoinNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
  if (!data.joinType) {
    errors.push({
      field: 'joinType',
...
// 출력 노드 검증
function validateOutputNode(data: OutputNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
  if (!data.outputType) {
    errors.push({
      field: 'outputType',
...
// 파이썬 노드 검증
function validatePythonNode(data: PythonNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
...
// Logpresso 노드 검증
function validateLogpressoNode(data: LogpressoNodeData, errors: ValidationError[], warnings: ValidationWarning[]): void {
  if (!data.query || data.query.trim() === '') {
    errors.push({
      field: 'query',
...
// 다이어그램 전체 검증
export function validateDiagram(diagram: Diagram): {
  isValid: boolean;
  ...
  const diagramErrors: ValidationError[] = [];
  const diagramWarnings: ValidationWarning[] = [];
...
// 노드 타입별 입력 포트 수 검증
export function validateNodeInputs(nodeType: NodeType, inputCount: number): boolean {
  switch (nodeType) {
    case NodeType.DATABASE:
    case NodeType.LOGPRESSO:
    case NodeType.PYTHON:
...
// 노드 타입별 출력 포트 수 검증
export function validateNodeOutputs(nodeType: NodeType, outputCount: number): boolean {
  switch (nodeType) {
    case NodeType.OUTPUT:
